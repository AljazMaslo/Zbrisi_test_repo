MODULE sbccpl
   ...
   USE sbcblk , ONLY : sf , jp_tair !Added by Aljaz
   ...
   INTEGER, PARAMETER ::   jps_ast = 39      ! air sea temperature difference (K)
   ...
   !                                   ! Send to waves
   TYPE(FLD_C) ::   sn_snd_ifrac, sn_snd_crtw, sn_snd_wlev, sn_snd_ast ! Aljaz added sn_snd_ast
   !                                   ! Received from waves
...
! Start added by Aljaz
      ...
      ! rto ...= 273.15 parameter to convert [Celsius] to [Kelvin]
               ...
               ztmp1(:,:) = (sf(jp_tair )%fnow(:,:,1)-rt0)- ts(:,:,1,jp_tem,Kmm)
